...
// MAGIC %python
// MAGIC # Full visualization of the linear regression exercise (can be done since there are only limited number of source data points)
...
// MAGIC training_data = spark.sql("SELECT X, label FROM training_data").collect()
// MAGIC test_data = spark.sql("SELECT X, label FROM test_data").collect()
// MAGIC test_predictions = spark.sql("SELECT X, prediction FROM test_predictions").collect()
// MAGIC
// MAGIC pyplot.xlabel("x")
// MAGIC pyplot.ylabel("y")
// MAGIC pyplot.xlim(-1.0, 12.0)
// MAGIC pyplot.ylim(-1.0, 17.0)
// MAGIC pyplot.plot([row[0] for row in training_data], [row[1] for row in training_data], 'bo')
// MAGIC pyplot.plot([row[0] for row in test_data], [row[1] for row in test_data], 'go')
// MAGIC pyplot.plot([row[0] for row in test_predictions], [row[1] for row in test_predictions], 'r-')
...
val staticDataPath: String = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex5/static/superstore_sales.csv"
...
val myStreamingIdentifier: String = "example_solution"

// setup the address for your folder in the students container in the Azure storage
val myStreamingFolder: String = s"abfss://students@tunics320f2025gen2.dfs.core.windows.net/ex5/${myStreamingIdentifier}/"
...
def copyFiles(targetFolder: String): Unit = {
    val streamingSource: String = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex5/streaming"
...
val myStreamingQuery: StreamingQuery = streamingBestDaysDF
    ...
    .outputMode("complete")
    .option("numRows", 8)
    .queryName(s"ex5_${myStreamingIdentifier}")
    .start()

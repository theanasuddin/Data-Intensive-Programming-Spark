...
def main():
    ...
    # The plot will be created into a PNG file "linear_regression.png"
    ...
    staticDataPath: str = "../../data/ex5/static/superstore_sales.csv"
    ...
    if not Path(myStreamingFolder).exists():
        Path(myStreamingFolder).mkdir(parents=True)
    ...
    def copyFiles(targetFolder: str) -> None:
        sourceFolder: str = "../../data/ex5/streaming"
        inputFileList: List[str] = sorted(glob.glob(f"{sourceFolder}/*"))

        for index, csvFile in enumerate(inputFileList):
            fileName: str = Path(csvFile).name
            inputFilePath: Path = Path(sourceFolder) / fileName

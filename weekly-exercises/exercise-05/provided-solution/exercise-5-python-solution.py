...
dataDF: DataFrame = spark.createDataFrame(dataRDD, dataSchema)
...
lr: LinearRegression = LinearRegression() \
    ...
    .setLabelCol("label")
    # setFeatureCol and setLabelCol are optional here, since "features" and "label" are the defaults
...
accuracy: float = testEvaluator.evaluate(testPredictions)

# COMMAND ----------

print(f"The RMSE for the model is {accuracy}")
...
newData: List[float] = [-2.8, 3.14, 9.9, 11.11, 22.22, 123.45]
newDataRows: List[Row] = [Row(value) for value in newData]
newDataSchema: StructType = StructType([StructField("X", DoubleType(), False)])
newDataFrame: DataFrame = spark.createDataFrame(spark.sparkContext.parallelize(newDataRows), newDataSchema)
...
newDataFrame2: DataFrame = spark.createDataFrame(newData, DoubleType()).toDF("X")

assembledNewDataDF: DataFrame = vectorAssembler.transform(newDataFrame)
newPredictions: DataFrame = lrModel.transform(assembledNewDataDF)
...
a: float = lrModel.coefficients[0]
b: float = lrModel.intercept

print("Prediction line: y = ax + b")
print(f"a = {a}")
print(f"b = {b}")

x_test: float = 123.45
y_test: float = a * x_test + b
print(f"Prediction for x = {x_test}  =>  y = {y_test}")
...
staticDataPath: str = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex5/static/superstore_sales.csv"
staticSalesDF: DataFrame = spark \
    .read \
    .option("header", True) \
    .option("sep", ";") \
...
staticBestDaysDF: DataFrame = staticSalesDF \
    .withColumn("totalPrice", F.col("productPrice") * F.col("productCount")) \
    .groupBy("orderDate") \
    .agg(F.sum("totalPrice").alias("totalSales")) \
...
myStreamingIdentifier: str = "example_solution"
...
streamingSalesDF: DataFrame = spark \
    .readStream \
    .option("header", False) \
    .option("sep", ";") \
...
streamingBestDaysDF: DataFrame = streamingSalesDF \
    .withColumn("totalPrice", F.col("productPrice") * F.col("productCount")) \
    .groupBy("orderDate") \
    .agg(
...
# By default, Spark divides the computation for shuffle operations (like groupBy) to 200 partitions
# For small datasets (and not having a large computation cluster) this can create unnecessary overhead.
# In this streaming task this overhead might cause the streaming aggregations to run very slowly.
# The default number of shuffle partitions can be set with the following configuration (in this case to 16):
# spark.conf.set("spark.sql.shuffle.partitions", 16)


# start the streaming query with the memory format and the query name set to the value of `streamQueryName`
myStreamingQuery: StreamingQuery = streamingBestDaysDF \
    .writeStream \
    .format("memory") \
    .outputMode("complete") \
...
# MAGIC %md
# MAGIC 1. RDDs vs DataFrames
# MAGIC     - RDDs can be used when you want to specify exactly what Spark should do with the data and how. RDDs can be an easier to start working with unstructured data than DataFrames. In Python, the map function is not available for DataFrames, and if you need to map an entire row using RDDs might be simpler than working with the methods provided by DataFrames. In addition, some legacy third-party APIs might require the use of RDDs.
# MAGIC     - DataFrames should be preferred by default. They provide optimizations and better performance compared to RDDs, i.e., DataFrames can optimize the transformations asked by the user while RDDs process the data in the way the user has instructed. For any structured data, it is likely better to handle them with DataFrames. With unstructured data, it might be reasonable to start with RDDs and then transform to DataFrame and handle the rest of the data processing using them.
# MAGIC     - RDDs can be transformed into DataFrames and vice versa, but the transformation will always cause some overhead and decreased performance.
# MAGIC 2. Streaming data
# MAGIC     - Static data is fully available at the start of the data processing, while streaming data arrives continuously and must be processed incrementally. By default, there is no end to the processing of streaming data.
# MAGIC     - There are some limitations with handling streaming data compared to static data: [https://spark.apache.org/docs/3.5.6/structured-streaming-programming-guide.html#unsupported-operations](https://spark.apache.org/docs/3.5.6/structured-streaming-programming-guide.html#unsupported-operations)
# MAGIC     - Some aggregations, joins, or sorting are either restricted or require careful state and watermark management.
# MAGIC     - For example, getting the total number of rows with .count() is not supported with streaming data frames. The same with trying to use .show() for printing the first few rows.
# MAGIC     - In addition, when dealing with streaming data, special consideration have to be made with fault tolerance and other streaming specific features.

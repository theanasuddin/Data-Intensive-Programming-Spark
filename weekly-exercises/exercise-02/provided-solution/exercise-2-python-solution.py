...
csvLocation: str = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex2/numbers/numbers.csv"
numberDF: DataFrame = spark \
    .read \
    .option("header", "true") \
    .option("delimiter", ",") \
...
weatherLocation: str = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex2/weather/nordics_weather.csv"
...
minTemp2: float = weatherDF.select(F.min("temperature_min")).take(1)[0][0]
maxTemp2: float = weatherDF.select(F.max("temperature_max")).head()[0]
...
weatherDFWithNewColumns: DataFrame = weatherDF \
    .withColumn("measurement_year", F.year(F.col("date"))) \
    .withColumn("measurement_weekday", F.weekday(F.col("date")))


# or
_2 = weatherDF \
    .withColumn("measurement_year", F.year(weatherDF["date"])) \
    .withColumn("measurement_weekday", F.weekday(weatherDF["date"]))


# alternative using SQL syntax
weatherDF.createOrReplaceTempView("weatherRelation2")
_3: DataFrame = spark.sql("""
...
yearlyTemperatureDF: DataFrame = weatherDFWithNewColumns \
    .groupBy(F.col("measurement_year").alias("year")) \
    .agg(
...
sqlTemperatureDF: DataFrame = spark.sql("""
...
sqlTemperatureDF2: DataFrame = weatherDFWithNewColumns \
    .groupBy("measurement_year") \
    .agg(F.min("temperature_min").alias("temperature_min")) \
    .join(
        weatherDFWithNewColumns.groupBy("measurement_year").agg(F.max("temperature_max").alias("temperature_max")),
        ["measurement_year"],
        "inner"
    ) \

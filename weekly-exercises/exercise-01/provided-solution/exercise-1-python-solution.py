...
# MAGIC
# MAGIC - The function uses `reduce` function to iterate through the list and calculate the sum of the elements and the element count at the same time.
...
# MAGIC     - The Python `reduce` is equivalent to Scala `foldLeft` when given the third parameter as the initial value. This allows the returned type to not have to match the types of the elements in the list.
...
# MAGIC
# MAGIC - If the input is an empty list, the average value calculation involves dividing by zero at the last line of the function. The Python implementation throws `ZeroDivisionError` in this case.

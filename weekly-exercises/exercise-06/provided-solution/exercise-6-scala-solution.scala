...
def folderSizeInKB(filePath: String) = sizeInKB((dbutils.fs.ls(filePath).map(_.size).sum))
...
def printStorage(path: String): Unit = {
    def getStorageSize(currentPath: String): Long = {
        ...
        val fileInformation: Seq[com.databricks.backend.daemon.dbutils.FileInfo] = dbutils.fs.ls(currentPath)
        fileInformation.map(
            fileInfo => {
                if (fileInfo.isDir) {
    ...
    val sizeInBytes: Long = getStorageSize(path)
...
val student_name: String = "special-unique-name"

val source_path: String = "abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex6/"
val target_path: String = s"abfss://students@tunics320f2025gen2.dfs.core.windows.net/ex6/${student_name}/"
...
val df_csv: DataFrame = spark
    ...
    .option("sep", "|")  // The files use "|" as the column separator
...
val df_parquet: DataFrame = spark
    .read
...
val df_new_rows_los_angeles: DataFrame = df_parquet
...
val df_new_rows_chicago: DataFrame = df_parquet
...
val new_rows_df: DataFrame = df_new_rows_los_angeles.union(df_new_rows_chicago)
...
val df_new_csv: DataFrame = spark
    .read
...
val df_new_parquet: DataFrame = spark
    .read
    .parquet(target_file_parquet)
...
val df_modified: DataFrame = new_rows_df  // The new (combined) rows from task 2
    .withColumn("ID", replace(col("ID"), lit("Z1"), lit("Z2")))
    .withColumn("AddedColumn1", concat(lit("prefix-"), col("City")))  // Add a new column based on City
    .withColumn("AddedColumn2", lit("New column"))                    // Add a new fixed column
    .withColumn("Temperature_F", (col("Temperature_F") - lit(32)) / lit(9) * lit(5))  // Replace the Fahrenheit values with Celsius values
    .withColumnRenamed("Temperature_F", "Temperature_C")              // Rename the temperature column
    .drop("Description")                                              // Remove a column
...
val modified_csv_df: DataFrame = spark
    .read
    .option("header", "true")
    .option("inferSchema", "true")
    .option("sep", "|")
...
// MAGIC     - The same output, depending on the run, both alternatives appeared.
// MAGIC
...
val modified_delta_df: DataFrame = spark
    .read
    .format("delta")
...
deltatable
    .as("orig")
    .merge(df_delta_update.as("updates"), "orig.ID = updates.ID") // Modifications
    .whenMatched().updateAll()          // When the primary key ID matches, update
    .whenNotMatched().insertAll()       // Insert new rows
    .execute()
...
val df_delta_second_update: DataFrame = deltatable.toDF
    .withColumn(
        "Temperature_C",
        round((col("Temperature_F") - lit(32)) / lit(9) * lit(5), 1)
    )
...
deltatable
    .as("orig")
    .merge(df_delta_second_update.as("updates"), "orig.ID = updates.ID") // Modifications
    .whenMatched().updateAll()          // When the primary key ID matches, update
    .execute()
...
// MAGIC %md
// MAGIC 1. CSV vs. Parquet
// MAGIC   - Parquet format is more efficient than CSV because it uses columnar storage and compression, which leads to faster reads and smaller file sizes, especially for large datasets.
// MAGIC   - CSV format is commonly supported by almost any applications. Their content can easily be inspected or edited manually by any text editor or applications like Excel. Parquet format does not have similar support, and to inspect the contents you have more limited options.
// MAGIC   - In general, with any large datasets Parquet should be preferred over CSV. With small datasets, or when the data needs to be used with non-Spark tools, CSV format is a viable option.
// MAGIC 2. Other formats
// MAGIC   - File formats supported by Spark natively include raw text, JSON, ORC, Avro, and XML (the last starting from version 4.0.0). Other formats like Delta Lake have support through additional libraries. Most commonly used file formats have support either natively or through libraries.
// MAGIC   - Yes, Spark can use other data sources besides files, such as relational databases (e.g., PostgreSQL, MySQL), NoSQL databases (e.g., Cassandra, MongoDB), and cloud storage systems (e.g., Azure Blob Storage, Amazon S3).
